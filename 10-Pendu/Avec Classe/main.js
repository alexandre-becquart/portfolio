...
class Pendu {
    ...
    init() {
        document.getElementById('generer').addEventListener("click", () => this.genererMot())
        // au clic sur l'el. generer, je génére un mot avec la FCT genererMot()

        document.getElementById('proLettre').addEventListener("click", () => this.proposerLettre())
        // au clic sur l'el. proLettre, je lance la FCT proposerLettre()

        document.getElementById('proMot').addEventListener("click", () => this.proposerMot())
    ...
    genererMot() {
        this.reset(); // la Fct est générer plus bas
        this.active(); // la Fct est générer plus bas

        this.wordRandom = Math.floor(Math.random() * this.words.length)
        // je génère un nombre que j'arrondis compris entre 0 et la longeur du tableau word.
        this.word = this.words[this.wordRandom]; // wordRandon devient l'index, j'acciche donc un mot
    ...
    proposerLettre() {
        this.wordSplit = this.word.split('') // je décompose mon mot afin d'avoir un tableau avec toutes les lettres
        //console.log(wordSplit);
        this.lettre = prompt("Proposez une lettre"); // je crée une prompt pour demander une lettre
        this.bonneLettre = false;


        if (this.compMot.indexOf(this.lettre) != -1) { // si la lettre existe déjà 
            alert("Cette lettre est déjà utilitée") // lance une alerte avec le message
        } else {

            for (var j = 0; j < this.wordSplit.length; j++) { // je boucle dans mon tableau pour vérifier les lettres

                if (this.lettre == this.wordSplit[j]) { // si la lettre existe dans le tableau
                    document.getElementsByClassName('lettre')[j].innerHTML = this.lettre;
                    // j'ajoute la lettre la div ayant le même index
                    this.bonneLettre = true; // il devient true pour eviter de passer dans le code erreur
                    this.compMot[j] = lettre; // il remet les lettres dans l'ordre dans le tableau
                    //console.log(compMot); // afficher le tableau 
                    if (this.compMot.join("") == this.wordSplit.join("")) {
                        //On recompose le mot du tableau 'compMot', s'il est égale au mot générer
                        this.gagne = document.getElementById('lettre');
                        this.gagne.classList.add('gagne');
                        this.gagne.innerHTML = "Bravo, vous avez gagné !!";
            ...
            if (!this.bonneLettre) { // si lettre est égale false
                alert("La lettre n'est pas bonne") //  envoie le message que la lettre est mauvaise
                this.etape++// ajoute +1 à l'étape 
                var div = document.createElement('div') // je crée des div par étape pour afficher le pendu 
                div.classList.add("etape" + this.etape)
                document.getElementById('partie2').appendChild(div) // la div crée prend la classe ayant le numéro de l'étape
                if (this.etape == 11) { // si etape est égale à 11 => le joueur a perdu
                    this.perdu = document.getElementById('lettre');
                    this.perdu.classList.add('perdu');
                    this.perdu.innerHTML = 'Désolé vous avez perdu!!'; // on affiche une div pour annoncer la fin du jeu
                    this.desactive();// je lance la fct desactive
                    for (var m = 0; m < this.wordSplit.length; m++) {
                        document.getElementsByClassName('lettre')[m].innerHTML = this.wordSplit[m];
